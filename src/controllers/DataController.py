    ...
    def validate_uplaoded_file(self, file: UploadFile):
        ...
        if file and file.size is not None:
            if file.content_type not in self.app_settings.FILE_ALLOWED_TYPES:
                return False, ResponseMessageEnums.FILE_TYPE_NOT_SUPPORTED.value

            if file.size > self.app_settings.FILE_MAX_SIZE * self.size_scale:
                return False, ResponseMessageEnums.FILE_SIZE_EXCEEDED.value

            return True, ResponseMessageEnums.FILE_UPLOAD_SUCCESS.value
        else:
            return False, ResponseMessageEnums.FILE_NOT_EXIST.value
